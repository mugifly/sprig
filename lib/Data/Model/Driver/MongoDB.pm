...
# Modified 2013-03-29: Support for new driver (by Masanori Ohgita)
# Modified 2013-04-04: Support order (order-by) (by Masanori Ohgita)
################################################################
...
sub _fetch {
    ...
    if ($multiple){
      my $r;
      eval {
        $r = $self->connector->get_database($db)->get_collection($collection)->find( $query->{ where } );
      };
      if ( $@ =~ /can't get db response, not connected/ ) {
        $r = $self->connector->get_database($db)->get_collection($collection)->find( $query->{ where } );
      }
      if($query->{order}){
        @res = $r->sort($query->{order})->all;
        } else {
        @res = $r->all;
      }
    } else {
      eval {
        @res = $self->connector->get_database($db)->get_collection($collection)->find_one( $query );
      };
      if ( $@ =~ /can't get db response, not connected/ ) {
        @res = $self->connector->get_database($db)->get_collection($collection)->find_one( $query );
      }
